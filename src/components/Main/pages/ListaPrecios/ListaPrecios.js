    ...
    <Paper className={classes.root}>
      <TableContainer className={classes.container}>
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {state.listaprecios.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => {
              return (
                ...
                tabIndex={-1} 
                key={row.code}
                 onDoubleClick={event =>  stkitemsreduc(event, row.StkRubroCodGrp, row.idStkRubro)}
                >
                  {state.columns.map(column => {
      ...
      />
<TablaMuestraStock open = {open} handleClose = {handleClose}  datositems = {state.datositems} />
