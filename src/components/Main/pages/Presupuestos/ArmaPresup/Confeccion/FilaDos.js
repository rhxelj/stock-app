...
import { presupcalculador } from '../../PresupVarios/PresupCalculador'
...
export default function FilaDos(props) {
  ...
  async function agregar() {
    ...
    if (state.renglonanexo.length !== 0) {
      ...
      // setState([ ...state, renglonpresup : state.renglonanexo])
      // setState([ ...state, renglonpresup : datospresup[0]])
      console.log('state  ', state.renglonpresup)
    }
    else {
      setDatosRenglon([...datosrenglon, datospresup[0]]);
      console.log('state  ', state.renglonpresup)
