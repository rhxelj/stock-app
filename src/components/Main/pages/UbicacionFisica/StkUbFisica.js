    ...
    render(){
        ...
        this.state.ub_fisica.map( (rowData,index) => 
            Object.assign(rowData, { 
                borrar: 
                    <StkUbFisicaBorrar idStkUbFisica={rowData.idStkUbFisica} StkUbFisicaGeo={rowData.StkUbFisicaGeo} read={()=>this.read()}></StkUbFisicaBorrar>})
        );
        //************************************** Agrego el campo del Boton BORRAR - End ***********************************  

       // ******************************************* Filtrado de datos - Begin *******************************************

        var ub_fisica = this.state.ub_fisica.filter((ubfisica) => { //este proveedores no es el proveedores de this.state.proveedores es una copia local
            return (
            ubfisica.idStkUbFisica.toLowerCase().indexOf(this.state.filtered.toLowerCase()) !== -1 
            ||
            ubfisica.StkUbFisicaGeo.toLowerCase().indexOf(this.state.filtered.toLowerCase()) !== -1
            // ||
                            ...
                            <div className="col s12 ">
                                <div className="">
                                    <div className="card-content  black-text">
                                        <StkUbFisicaAgregar toggleAgregar={this.toggleAgregar} read={this.read}> </StkUbFisicaAgregar>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <Paper >
                        <Table >
                            ...
                            <TableBody>
                                {ub_fisica.map(row => {
                                    return (
                                        <TableRow className={this.props.classes.row} key={row.idStkUbFisica}
                                            onDoubleClick={() => {
