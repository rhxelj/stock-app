...
var initial_state = {
  ...
  stkrubroele: [],
  // StkItemsGrupo: 0,
  StkItemsCantidad: null,
  StkItemsCantDisp: null,
  StkItemsFAct: "",
  StkItemsMin: null,
  StkItemsMax: null,
  StkRubroAncho: null,
...
var StkMovEntrada = props => {
  ...
  const stkrubroleecodgryrb = () => {
    console.log("esta en stkrubroleecodgryrb  ", state.idStkGrupo);
    if (state.idStkGrupo != 0 && state.idStkRubro != 0) {
      const url =
        IpServidor +
      ...
      request
        ...
        .then(res => {
          ...
          // console.log("state.StkRubroPresDes ", stkrubroele[0].StkRubroPresDes);
          setState({
            ...state,
  ...
  useEffect(() => {
    console.log("esta en el stkgrupoleer");

    stkgrupoleer(); //leo grupos
  }, [state.limpiapantalla]);

  useEffect(() => {
    if (state.idStkGrupo != "") {
      console.log("esta en el stkrubroleecodgrupo");
      stkrubroleecodgrupo(state.idStkGrupo); //leo rubros apartir del grupo seleccionado
    }
  }, [state.idStkGrupo, state.limpiapantalla]);

  useEffect(() => {
    // if (state.idStkGrupo != null && state.idStkRubro != null) {
    stkrubroleecodgryrb();
    // }
  }, [state.idStkGrupo, state.idStkRubro]);

  useEffect(() => {
    // if (state.idStkRubro != "") { (lo saqué porque creo que tiene que tener valor los dos)
    // if (state.idStkGrupo != "" && state.idStkRubro != "") {
    stkitemsleecodgryrb(); //leo items apartir del grupo y rubro seleccionados
    // }
  }, [state.idStkRubro, state.idStkItems]);
  ...
  // useEffect(() => {
  //   if (state.limpiapantalla) {
  //     console.log("limpia ");
  //     setState(state.limpiapantalla);
  //   }
  // });
    ...
    <div>
      ...
      <Dialog open={true} maxWidth="xs" fullWidth={true}>
        ...
        <DialogContent>
          <Grid container spacing={2}>
            ...
            <Grid container spacing={2} justify="flex-start">
              ...
              {/* agregado para que muestre que presentación */}
              <Grid item xs={3}>
                <TextField
                  label=" "
                  id="StkRubroPresDes"
        ...
        <DialogActions>
          ...
          {console.log("state.limpiapantalla ", state.limpiapantalla)}
          <Button
            variant="contained"
            color="secondary"
            onClick={limpioPantalla}
            // onClick={() => setState(state.limpiapantalla)}
          >
            Cancelar
          </Button>
        </DialogActions>
